...
This project builds a **synthetic wholesale credit portfolio** of 1,000 counterparties and applies multiple risk frameworks to each exposure: Basel III Pillar‑I capital, CCAR‑style regulatory stress, IFRS‑9 expected credit loss, and an internal severe stress scenario. The aim is to demonstrate how capital and expected losses change under different stress assumptions and which segments (industry, country, rating) are most sensitive.
...
- (3) estimates IFRS‑9 lifetime expected credit losses at counterparty and segment level, in order to compare baseline vs stressed risk measures and identify segments that drive the largest capital and loss sensitivity.

---

## Dataset

- Source: Fully synthetic, generated in Python (no real customer data).
- Portfolio size: 1,000 counterparties (`CounterpartyID` 1–1000).

Key columns:

- `CounterpartyID` – Unique ID per obligor.
- `EAD` – Exposure at Default (continuous, 1e6–1e9). 
- `PD` – Probability of Default (1%–20%).
- `LGD` – Loss Given Default (20%–80%).
- `Industry` – Banking & Financial Services, Manufacturing, Services, Retail, Technology, Energy & Utilities, Real Estate & Construction. 
- `Country` – US, UK, Germany, France, Switzerland, China, South Korea. 
- `Rating` – AAA, AA, A, BBB, BB, B, CCC, CC, C, D. 

Derived risk measures:

- `Pillar_I_Capital_Requirement` – Basel III Pillar‑I capital.  
- `Stressed_PD`, `Stressed_LGD`, `Capital_Requirement_CCAR` – CCAR stress capital.  
- `Expected_Credit_Loss_IFRS9` – IFRS‑9 ECL. 
- `Stressed_PD_Internal`, `Stressed_LGD_Internal`, `Capital_Requirement_Internal_Stress_Test` – internal severe stress capital.  
- `Perc_Increase_Baseline_to_Severe` – % increase from Pillar‑I to internal severe stress. 

---

## Tools & Technologies

- **Python (pandas, numpy)** – Data simulation and risk calculations.  
- **Jupyter Notebook** – Interactive analysis and visualization.
- **Matplotlib / Seaborn** – Charts for portfolio and risk metrics.
...
Main modeling steps performed in Python: 
...
- **Basel III Pillar‑I capital**  
  - Formula per counterparty:  
    - `Capital}_Pillar I` = EAD * PD * LGD * 12.5
...
- **CCAR stress scenario**  
  ...
  - `Capital_CCAR = EAD * Stressed_PD * Stressed_LGD * 12.5`
  - Implemented in `add_ccar_stress(df)`.  

- **IFRS‑9 Expected Credit Loss**  
  - ECL_IFRS9 = EAD * PD * LGD
...
- **Internal severe stress**  
  ...
  - `Capital_Internal = EAD * Stressed_Internal * Stressed_LGD_Internal * 12.5
    
  - **Severity metric:**  
    - `Perc_Increase_Baseline_to_Severe = {(Capital_Internal - Capital_Pillar 1) / Capital_Pillar 1} * 100`
